...
def startup():
    # Default slots from add-on options are stored via settings on init
    # If db exists already, init_db is idempotent.
...
def ui_meals_add(
    ...
    with get_db() as con:
        con.execute("INSERT OR IGNORE INTO meals(name, notes) VALUES(?,?)", (name.strip(), notes.strip()))
    return RedirectResponse(url=f"{INGRESS_PATH}/meals", status_code=303)
...
def ui_meal_ingredient_set(
    ...
    with get_db() as con:
        ...
        VALUES(?,?,?)
        ON CONFLICT(meal_id, ingredient_id) DO UPDATE SET amount_per_person=excluded.amount_per_person
...
def ui_planner(request: Request, start: str | None = None):
    ...
    with get_db() as con:
        ...
        """, (start_date.isoformat(), (start_date + timedelta(days=6)).isoformat())).fetchall()
...
def ui_planner_set(
    ...
    with get_db() as con:
        ...
        VALUES(?,?,?,?)
        ON CONFLICT(day, slot) DO UPDATE SET meal_id=excluded.meal_id, servings=excluded.servings
        """, (day, slot, meal_id if meal_id else None, int(servings)))
    return RedirectResponse(url=f"{INGRESS_PATH}/planner?start={day}", status_code=303)
